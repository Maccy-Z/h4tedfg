...
def main():
    ...
    gp = Main.make_GP(X, y, n_class=2, init_sigma=1., init_scale=1.)

    # Train the GP
    vars, inv_lens = Main.train_GP(gp, n_iter=100)
    ...
    (probs, p_std), (f_mu, f_var) = Main.pred_proba(gp, X_test, diag=True, model_cov=True)

    f_mu, f_std = np.array(f_mu), np.sqrt(np.array(f_var))
