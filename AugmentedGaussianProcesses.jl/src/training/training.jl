...
function kernel_fn(kern, x)
    # return cholesky(kernelmatrix(kernel(gp), x) + 1e-4 * I)
    kern_mat = kernelmatrix(kern, x)

    return cholesky(kern_mat + 1e-4 * I)
end
...
#         kernel_matrices = [a, b, c]
        ...
        # setHPupdated!(inference(m), false)
        # return_state = merge(state, (; kernel_matrices))
        return kernel_matrices# return_state
    else
